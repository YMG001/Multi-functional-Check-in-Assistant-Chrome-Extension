...
2. **选择器配置**
   - 签到按钮选择器：
     - 选择器类型：支持XPath/CSS/JavaScript/正则表达式
     - 选择器内容：用于定位签到按钮
   - 已签到标识选择器：
     - 选择器类型：独立配置，支持XPath/CSS/JavaScript/正则表达式
     - 选择器内容：用于检测签到状态

3. **高级设置**
   - 关闭等待时间：签到成功后的等待时间
   - 自定义JavaScript：支持复杂的签到逻辑
   - 显示签到过程：控制是否显示签到操作的页面
...
3. **JavaScript选择器**
   - 示例：`document.querySelector('.sign-button')`
   - 适用：需要动态计算的元素
